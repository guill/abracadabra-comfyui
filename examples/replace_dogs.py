# Instruction: Replace all dogs with beavers
# Available locals:
# - g: GraphBuilder
# - RAND: fn() -> int
...
dog_mask_raw = g.node("Mask By Text", image=input1, prompt="dog", negative_prompt="", precision=0.3, normalize="no")
# Make sure we cut out the entire face
dog_open = g.node("Mask Morphology", image=dog_mask_raw.out(0), op="open", distance=10)
dog_dilated = g.node("Mask Morphology", image=dog_open.out(0), op="dilate", distance=8)
# Inpainting requires an input of type "MASK", so we'll use Image to Mask to convert the image to a mask.
inpaint_mask = g.node("Image To Mask", image=dog_dilated.out(0), method="intensity")
...
# Using the syntax of '(prompt:1.1)' emphasizes the prompt's importance
prompt = g.node("CLIPTextEncode", clip=loader.out(1), text="(pet beaver:1.1)")
negative = g.node("CLIPTextEncode", clip=loader.out(1), text="dog")
# Use 80 steps for better inpainting
sampler = g.node("KSampler", model=loader.out(0), positive=prompt.out(0), negative=negative.out(0), latent_image=vae_encode.out(0), seed=RAND(), steps=80, cfg=8.0, sampler_name="euler", scheduler="normal", denoise=1.0,)
